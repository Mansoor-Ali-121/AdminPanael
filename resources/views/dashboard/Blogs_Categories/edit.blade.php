        ...
        <div class="container">
            <div class="row">
                <div class="col-12 text-center">
                    <h1 style="font-weight: bold">Edit <span style="color: #ff7b23f8">Category</span></h1>
                    <p class="lead">Update information about this category</p>
                </div>
            </div>
        </div>
    </header>

    <form action="{{ route('category.update', $category->category_id) }}" method="POST"> {{-- Update route --}}
        @csrf
        @method('PATCH')
        ...
        <section class="card p-3 mb-3 card-border">
            ...
            <fieldset class="form-group">
                ...
                <input type="text" id="actual_slug" name="actual_slug" class="form-control"
                    value="{{ old('actual_slug', $category->category_slug) }}" onkeyup="generateSlug()" required>
                @error('actual_slug')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </fieldset>
